...
{
    /*
     * http://slks.dk/om-slots-og-kulturstyrelsen/kulturarvsdatabaserne/til-udviklere/webservices/
     * http://www.kulturarv.dk/ffpublic/wfs?version=1.0.0 
     */
    
    public class KulturarvRepository : IKulturarvRepository
    {
        private static readonly ILogger Logger = LogManager.Instance.GetLogger("Niras.Jordflytning.Infrastructure.DataAccess.KulturarvRepository");
        private const string HeaderText = "Slots- og Kulturstyrelsen";
        private readonly string _wfsUrl;

        public KulturarvRepository()
        {
            _wfsUrl = ConfigurationManager.AppSettings["kulturarvWfsUrl"];
        }
        ...
        public KonfliktSoegningLeverandoerResultat KonfliktSoegningMidlertidigModtager(DbGeometry geom)
        {
            ...
            {
                new LayerHandler("fundogfortidsminder_areal_beskyttelse", "Fortidsminder, beskyttede arealer"),
            };
            ...
            try
            {
                ...
                foreach (var layerHandler in layerHandlers)
                {
                    ...
                    {
                        error = ex;
                        Logger.LogException(string.Format("KulturarvRepository.GetResultMidlertidigModtager -> Service kald fejlede for lag: '{0}' - Url: '{1}'", layerHandler, url), ex);
                    }
            ...
            {
                Logger.LogException("KulturarvRepository.GetResultMidlertidigModtager", exception);
        ...
        private bool TjekReultatForHit(XDocument xDoc, string serviceName)
        {
            ...
            if (first != null && first.Name.LocalName != "FeatureCollection")
                throw new Exception("Systemfejl: Kulturarv: " + first.Value);
        ...
        private string CreateInputParamPost(DbGeometry dbGeom, string serviceName)
        {
            ...
            var requestStr =
                ...
                @"	maxFeatures='50' " +
                @"	handle='' >" +
                @"		<Query typeName='" + serviceName + "' srsName='EPSG:25832' >" +
                @"			" + filter +
                @"		</Query>" +
                @"</GetFeature>";
